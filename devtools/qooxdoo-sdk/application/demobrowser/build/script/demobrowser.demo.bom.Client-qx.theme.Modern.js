...
qx.Class.define(b,{statics:{AVAILABLE:false,FULLVERSION:"0.0.0",REVISION:"0",VERSION:0.0,EXPRESSINSTALL:false,STRICT_SECURITY_MODEL:false,_cachedSupportsVersion:{},supportsVersion:function(p){if(typeof this._cachedSupportsVersion[p]===j){return this._cachedSupportsVersion[p];
}else{var s=p.split(m);
var r=this.FULLVERSION.split(m);

for(var i=0;i<s.length;i++){var q=parseInt(r[i],10)-parseInt(s[i],10);

if(q>0){return (this._cachedSupportsVersion[p]=true);
}else if(q<0){return (this._cachedSupportsVersion[p]=false);
}}return (this._cachedSupportsVersion[p]=true);
}},__wN:qx.core.Variant.select(c,{"mshtml":function(){if(!window.ActiveXObject){return;
}var v=[0,0,0];
var u=false;

try{var w=new ActiveXObject(d);
}catch(x){try{var w=new ActiveXObject(e);
v=[6,0,21];
w.AllowScriptAccess=f;
}catch(y){if(v[0]==6){u=true;
}}
if(!u){try{w=new ActiveXObject(a);
}catch(z){}}}
if(!u&&typeof w==k){var t=w.GetVariable(g);

if(typeof t!=l){t=t.split(" ")[1].split(h);
v[0]=parseInt(t[0],10);
v[1]=parseInt(t[1],10);
v[2]=parseInt(t[2],10);
}}this.__wO(v);
...
var B=navigator.plugins[n].description;

if(typeof B!=l){B=B.replace(/^.*\s+(\S+\s+\S+$)/,o);
A[0]=parseInt(B.replace(/^(.*)\..*$/,o),10);
A[1]=parseInt(B.replace(/^.*\.(.*)\s.*$/,o),10);
A[2]=/r/.test(B)?parseInt(B.replace(/^.*r(.*)$/,o),10):0;
}this.__wO(A);
}}),__wO:function(C){this.FULLVERSION=C.join(".");
this.VERSION=parseFloat(C);
this.AVAILABLE=this.VERSION>0;
this.REVISION=C[C.length-1];

if(C[0]<10){this.STRICT_SECURITY_MODEL=this.supportsVersion("9.0.151");
}else{this.STRICT_SECURITY_MODEL=this.supportsVersion("10.0.12");
}var D=qx.bom.client.Platform;
this.EXPRESSINSTALL=(D.WIN||D.MAC)&&this.supportsVersion("6.0.65");
}},defer:function(E){E.__wN();
}});
})();
