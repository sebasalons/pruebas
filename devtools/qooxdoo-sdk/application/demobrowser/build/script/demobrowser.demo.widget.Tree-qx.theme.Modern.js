...
this._addWidgets();
this.initOpen();
},properties:{open:{check:E,init:false,event:d,apply:w},openSymbolMode:{check:[H,o,j],init:j,event:B,apply:A},indent:{check:D,init:19,apply:C,themeable:true},parent:{check:G,nullable:true},icon:{check:g,apply:v,nullable:true,themeable:true},iconOpened:{check:g,apply:F,nullable:true,themeable:true},label:{check:g,apply:x,init:t}},members:{__pm:null,__pn:null,__po:null,__pp:null,__pq:null,__pr:null,_addWidgets:function(){throw new Error("Abstract method call.");
...
if(this.__pp){this._remove(P);
}this._add(P);
this.__pp=true;
},addLabel:function(Q){var R=this.getChildControl(f);

if(this.__po){this._remove(R);
}
...
}}},_applyIcon:function(Y,ba){if(!this.__pt()){this.__pu(Y);
}else if(!this.isOpen()){this.__pu(Y);
}},_applyIconOpened:function(bb,bc){if(this.isOpen()){if(this.__ps()&&this.__pt()){this.__pu(bb);
}else if(!this.__ps()&&this.__pt()){this.__pu(bb);
...
if(bg){bi=this.__pt()?this.getIconOpened():null;
}else{bi=this.getIcon();
}
if(bi){this.__pu(bi);
}bg?this.addState(a):this.removeState(a);
},__ps:function(){return qx.util.PropertyUtil.getUserValue(this,k);
},__pt:function(){return qx.util.PropertyUtil.getUserValue(this,p);
},__pu:function(bj){var bk=this.getChildControl(k,true);
...
if(this.__pq){this.__pq.setWidth((this.getLevel()+1)*this.getIndent()-bq);
...
},getChildrenContainer:function(){if(!this.__pn){this.__pn=new qx.ui.container.Composite(new qx.ui.layout.VBox()).set({visibility:this.isOpen()?b:c});
}return this.__pn;
...
if(bL==this.__pm.length){this.add(bK);
return;
}var bP=bK.getParent();
...
}qx.lang.Array.insertAt(this.__pm,bK,bL);
...
if(bS){bS.remove(bQ);
}this.addAt(bQ,this.__pm.indexOf(bR));
},addAfter:function(bT,bU){{};
var bV=bT.getParent();

if(bV){bV.remove(bT);
}this.addAt(bT,this.__pm.indexOf(bU)+1);
},addAtBegin:function(bW){this.addAt(bW,0);
},remove:function(bX){for(var i=0,l=arguments.length;i<l;i++){var cc=arguments[i];

if(this.__pm.indexOf(cc)==-1){this.warn("Cannot remove treeitem '"+cc+"'. It is not a child of this tree item.");
return;
...
}}qx.lang.Array.remove(this.__pm,cc);
cc.setParent(null);
bY.remove(cc);
}var ca=this.getTree();
...
},removeAt:function(cd){var ce=this.__pm[cd];
