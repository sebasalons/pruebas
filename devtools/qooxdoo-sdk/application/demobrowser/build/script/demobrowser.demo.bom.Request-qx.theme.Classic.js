...
var settings = {"qx.allowUrlSettings":"true","qx.allowUrlVariants":"true","qx.application":"demobrowser.demo.bom.Request","qx.theme":"qx.theme.Classic"};
...
qx.$$loader = {
  parts : {"boot":[0]},
  uris : [["__out__:demobrowser.demo.bom.Request-qx.theme.Classic.js"]],
  urisBefore : [],
  packageHashes : {"0":"c0a1672dd6f2"},
  boot : "boot",
  closureParts : {},
  bootIsInline : true,
...
qx.Class.define(c,{extend:qx.core.Object,construct:function(){this.__wa={};
this.__wb=this.__wl();
},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{__wc:null,__wd:null,__wb:null,__we:null,__wf:null,__wg:null,__wh:null,__wa:null,readyState:0,responseText:f,responseXML:null,status:0,statusText:f,timeout:0,onreadystatechange:function(){},ontimeout:function(){},onload:function(){},onerror:function(){},onabort:function(){},open:function(j,k,l,m,n){if(l==null){l=true;
}this.__wc=l;
this.__wd=qx.lang.Function.bind(this.__wi,this);
this.__we=qx.lang.Function.bind(this.__wj,this);
this.__wb.onreadystatechange=this.__wd;
if(arguments.length>4){this.__wb.open(j,k,l,m,n);
}else if(arguments.length>3){this.__wb.open(j,k,l,m);
}else{this.__wb.open(j,k,l);
}if(qx.core.Variant.isSet(i,h)){if(!l){this.readyState=qx.bom.Request.OPENED;
this.__wm();
}}},send:function(o){var p=this.__wa;
if(o&&o.nodeType){o=window.XMLSerializer?new XMLSerializer().serializeToString(o):o.xml;

if(!p[g]){p[g]=d;
}}for(var q in p){this.__wb.setRequestHeader(q,p[q]);
}if(this.timeout!=null&&this.timeout>0){this.__wf=window.setTimeout(this.__we,this.timeout);
}this.__wb.send(o);
if(qx.core.Variant.isSet(i,h)){if(!this.__wc){this.readyState=qx.bom.Request.OPENED;
this.__wo(this);
...
},abort:function(){if(this.__wf){window.clearTimeout(this.__wf);
}this.__wk();
this.onabort();
this.dispose();
},__wi:qx.event.GlobalError.observeMethod(function(){if(qx.core.Variant.isSet(i,h)){if(!this.__wc){return;
}}this.readyState=this.__wb.readyState;
this.__wo();
if(this.__wg){this.readyState=qx.bom.Request.UNSENT;
return ;
}if(this.readyState==qx.bom.Request.DONE&&this.__wf){window.clearTimeout(this.__wf);
}this.__wm();
if(this.readyState==qx.bom.Request.DONE){this.dispose();
}}),__wj:qx.event.GlobalError.observeMethod(function(){this.__wk();
this.ontimeout();
this.dispose();
}),getAllResponseHeaders:function(){try{return this.__wb.getAllResponseHeaders();
}catch(r){return null;
}},getResponseHeader:function(s){try{return this.__wb.getResponseHeader(s);
}catch(t){return null;
}},setRequestHeader:function(u,v){if(v==null){delete this.__wa[u];
}else{this.__wa[u]=v;
}},removeRequestHeader:function(w,x){delete this.__wa[w];
},getRequestHeader:function(y){return this.__wa[y]||null;
},__wk:function(){this.__wo();
if(this.readyState>qx.bom.Request.UNSENT){this.__wg=true;
}this.__wb.abort();
},__wl:qx.core.Variant.select(i,{"default":function(){return new XMLHttpRequest;
},"mshtml":function(){if(window.ActiveXObject&&qx.xml.Document.XMLHTTP){return new ActiveXObject(qx.xml.Document.XMLHTTP);
}
if(window.XMLHttpRequest){return new XMLHttpRequest;
}}}),__wm:function(){if(this.__wh===this.readyState){return;
}this.onreadystatechange();
this.__wh=this.readyState;
if(this.readyState===4){if(this.isSuccessful()){this.onload();
}else{this.onerror();
}}},__wn:function(){var z=this.__wb.responseXML;

if(qx.core.Variant.isSet(i,e)){if(z&&!z.documentElement&&this.__wb.getResponseHeader(g).match(/[^\/]+\/[^\+]+\+xml/)){z=new ActiveXObject(qx.xml.Document.DOMDOC);
z.loadXML(this.__wb.responseText);
}if(z&&z.parseError!=0){return null;
}}else if(!z.documentElement||z.documentElement.tagName==a){return null;
}return z;
},__wo:function(){var A=this.__wb;

try{this.responseText=A.responseText;
}catch(B){}
try{this.responseXML=this.__wn();
}catch(C){}
...
}}},destruct:function(){if(this.__wf){window.clearTimeout(this.__wf);
this.__wf=null;
}if(this.__wb){this.__wb.onreadystatechange=qx.lang.Function.empty;
this.__wb=null;
