...
(function(){var k="execute",j="Flash Player Version",i="right",h="qooxdoo is the best!",g="Message from Flash:",f="Simple demo with FlashVars",d="bgcolor",c="demobrowser.demo.widget.Flash",b="__Ag",a="demobrowser/demo/flash/TestFlash.swf",C="Send to Flash",B="demobrowser/demo/flash/FlashVersion.swf",A="No message.",z="Flash:",y="<b>qooxdoo:</b>",x="__Af",w="__Aj",v="demobrowser.demo.widget.Flash.sendMessage",u="demobrowser.demo.widget.Flash.flashReady",t="__Ah",r="demobrowser/demo/flash/fo_tester.swf",s="this is passed in via FlashVars",p="Two way communication (ActionScript <-> JavaScript)",q="Message to Flash:",n="Show Flash Player version",o="noscale",l="#FF6600",m="__Ai";
qx.Class.define(c,{extend:qx.application.Standalone,members:{__Af:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var D=new qx.ui.tabview.TabView();
D.setContentPadding([8,8,8,8]);
var G=this.getRoot();
G.add(D,{edge:0});
var E=new qx.ui.tabview.Page(f);
E.setLayout(new qx.ui.layout.Canvas());
E.add(this.createFlashDemo1(),{edge:0});
D.add(E);
var F=new qx.ui.tabview.Page(p);
F.setLayout(new qx.ui.layout.Canvas());
F.add(this.createFlashDemo2(),{edge:0});
D.add(F);
this.__Af=this.createWindow();
G.add(this.__Af,{top:20,left:20});
...
H.add(new qx.ui.embed.Flash(B),{edge:0});
return H;
},createFlashDemo1:function(){var K=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
var I={flashVarText:s};
var J=new qx.ui.embed.Flash(r).set({scale:o,variables:I});
J.getContentElement().setParam(d,l);
K.add(J,{flex:1});
var L=new qx.ui.form.Button(n);
L.addListener(k,function(){this.__Af.open();
},this);
K.add(L);
return K;
},__Ag:null,__Ah:null,__Ai:null,__Aj:null,createFlashDemo2:function(){this.self(arguments).setCallBackInstance(this);
var O=new qx.ui.container.Composite(new qx.ui.layout.HBox(8));
var N=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
O.add(N,{flex:1});
N.add(new qx.ui.basic.Label(y).set({rich:true}));
N.add(new qx.ui.basic.Label(g));
this.__Ah=new qx.ui.form.TextArea(A);
this.__Ah.setEnabled(false);
N.add(this.__Ah,{flex:1});
N.add(new qx.ui.basic.Label(q));
this.__Ai=new qx.ui.form.TextArea(h);
N.add(this.__Ai,{flex:1});
var P=new qx.ui.container.Composite(new qx.ui.layout.HBox(0,i));
this.__Aj=new qx.ui.form.Button(C);
this.__Aj.setEnabled(false);
this.__Aj.addListener(k,function(e){this.__Ag.getFlashElement().sendMessage(this.__Ai.getValue());
},this);
P.add(this.__Aj);
N.add(P);
var M=new qx.ui.container.Composite(new qx.ui.layout.VBox(4));
O.add(M,{flex:1});
M.add(new qx.ui.basic.Label(z).set({rich:true}));
this.__Ag=new qx.ui.embed.Flash(a);
this.__Ag.setVariables({init:u});
M.add(this.__Ag,{flex:1});
return O;
},sendMessage:function(Q){this.__Ah.setValue(Q);
},initFlash:function(){if(this.__Ag.getFlashElement().setup){this.__Ag.getFlashElement().setup(v);
this.__Aj.setEnabled(true);
...
}}},statics:{__Ak:null,setCallBackInstance:function(R){demobrowser.demo.widget.Flash.__Ak=R;
},sendMessage:function(S){demobrowser.demo.widget.Flash.__Ak.sendMessage(S);
},flashReady:function(){demobrowser.demo.widget.Flash.__Ak.initFlash();
}},destruct:function(){this._disposeObjects(t,m,w,b,x);
}});
})();
...
(function(){var l="move",k="Boolean",j="mouseup",i="mousedown",h="losecapture",g="__ti",f="qx.ui.core.MMovable",d="__th",c="mousemove",b="maximized",a="move-frame";
qx.Mixin.define(f,{properties:{movable:{check:k,init:true},useMoveFrame:{check:k,init:false}},members:{__th:null,__ti:null,__tj:null,__tk:null,__tl:null,__tm:null,__tn:null,__to:false,__tp:null,__tq:0,_activateMoveHandle:function(m){if(this.__th){throw new Error("The move handle could not be redefined!");
}this.__th=m;
...
m.addListener(h,this.__tu,this);
},__tr:function(){var n=this.__ti;

if(!n){n=this.__ti=new qx.ui.core.Widget();
n.setAppearance(a);
n.exclude();
qx.core.Init.getApplication().getRoot().add(n);
}return n;
},__ts:function(){var location=this.getContainerLocation();
var p=this.getBounds();
var o=this.__tr();
o.setUserBounds(location.left,location.top,p.width,p.height);
o.show();
o.setZIndex(this.getZIndex()+1);
},__tt:function(e){var r=this.__tj;
var u=Math.max(r.left,Math.min(r.right,e.getDocumentLeft()));
var t=Math.max(r.top,Math.min(r.bottom,e.getDocumentTop()));
var q=this.__tk+u;
var s=this.__tl+t;
return {viewportLeft:q,viewportTop:s,parentLeft:q-this.__tm,parentTop:s-this.__tn};
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__tp=parent.getBlockerColor();
this.__tq=parent.getBlockerOpacity();
parent.setBlockerColor(null);
parent.setBlockerOpacity(1);
parent.blockContent(this.getZIndex()-1);
this.__to=true;
}}this.__tj={left:w.left,top:w.top,right:w.left+x.width,bottom:w.top+x.height};
var v=this.getContainerLocation();
this.__tm=w.left;
this.__tn=w.top;
this.__tk=v.left-e.getDocumentLeft();
this.__tl=v.top-e.getDocumentTop();
this.addState(l);
this.__th.capture();
if(this.getUseMoveFrame()){this.__ts();
}e.stop();
},_onMoveMouseMove:function(e){if(!this.hasState(l)){return;
}var y=this.__tt(e);

if(this.getUseMoveFrame()){this.__tr().setDomPosition(y.viewportLeft,y.viewportTop);
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__to){parent.unblockContent();
parent.setBlockerColor(this.__tp);
parent.setBlockerOpacity(this.__tq);
this.__tp=null;
this.__tq=0;
this.__to=false;
}}this.__th.releaseCapture();
var z=this.__tt(e);
this.setLayoutProperties({left:z.parentLeft,top:z.parentTop});
if(this.getUseMoveFrame()){this.__tr().exclude();
}e.stopPropagation();
},__tu:function(e){if(!this.hasState(l)){return;
}this.removeState(l);
if(this.getUseMoveFrame()){this.__tr().exclude();
...
qx.Mixin.define(r,{construct:function(){this.addListener(i,this.__tH,this,true);
this.addListener(q,this.__tI,this);
this.addListener(x,this.__tK,this);
this.addListener(s,this.__tL,this);
this.addListener(o,this.__tJ,this);
var A=this.getContainerElement().getDomElement();

if(A==null){A=window;
}this.__tv=qx.event.Registration.getManager(A).getHandler(qx.event.handler.DragDrop);
},properties:{resizableTop:{check:k,init:true},resizableRight:{check:k,init:true},resizableBottom:{check:k,init:true},resizableLeft:{check:k,init:true},resizable:{group:[l,d,m,y],mode:v},resizeSensitivity:{check:a,init:5},useResizeFrame:{check:k,init:true}},members:{__tv:null,__tw:null,__tx:null,__ty:null,__tz:null,__tA:null,__tB:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,__tC:function(){var B=this.__tw;

if(!B){B=this.__tw=new qx.ui.core.Widget();
B.setAppearance(p);
B.exclude();
qx.core.Init.getApplication().getRoot().add(B);
}return B;
},__tD:function(){var D=this.__tA;
var C=this.__tC();
C.setUserBounds(D.left,D.top,D.width,D.height);
C.show();
C.setZIndex(this.getZIndex()+1);
},__tE:function(e){var F=this.__tx;
...
if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){K=Math.max(J.top,Math.min(J.bottom,e.getDocumentTop()))-this.__tz;
...
if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){K=Math.max(J.left,Math.min(J.right,e.getDocumentLeft()))-this.__ty;
...
},__tF:{1:f,2:n,4:h,8:z,5:t,6:g,9:c,10:b},__tG:function(e){var O=this.getContentLocation();
...
},__tH:function(e){if(!this.__tx){return;
}this.addState(j);
this.__ty=e.getDocumentLeft();
this.__tz=e.getDocumentTop();
var location=this.getContainerLocation();
var T=this.getBounds();
this.__tA={top:location.top,left:location.left,width:T.width,height:T.height,bounds:qx.lang.Object.clone(T)};
var parent=this.getLayoutParent();
var R=parent.getContentLocation();
var S=parent.getBounds();
this.__tB={left:R.left,top:R.top,right:R.left+S.width,bottom:R.top+S.height};
if(this.getUseResizeFrame()){this.__tD();
}this.capture();
e.stop();
},__tI:function(e){if(!this.hasState(j)){return;
}if(this.getUseResizeFrame()){this.__tC().exclude();
}var U=this.__tE(e);
...
this.releaseCapture();
e.stopPropagation();
},__tJ:function(e){if(!this.__tx){return;
}this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
this.removeState(w);
if(this.getUseResizeFrame()){this.__tC().exclude();
}},__tK:function(e){if(this.hasState(j)){var Y=this.__tE(e);
if(this.getUseResizeFrame()){var W=this.__tC();
...
}else if(!this.hasState(u)&&!this.__tv.isSessionActive()){this.__tG(e);
var ba=this.__tx;
var X=this.getApplicationRoot();

if(ba){var V=this.__tF[ba];
this.setCursor(V);
X.setGlobalCursor(V);
}else if(this.getCursor()){this.resetCursor();
X.resetGlobalCursor();
}}},__tL:function(e){if(this.getCursor()&&!this.hasState(j)){this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
}}},destruct:function(){if(this.__tw!=null&&!qx.core.ObjectRegistry.inShutDown){this.__tw.destroy();
this.__tw=null;
}this.__tv=null;
}});
})();
...
(function(){var b="__tM",a="qx.ui.window.Manager";
qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__tM:null,setDesktop:function(c){this.__tM=c;
this.updateStack();
},getDesktop:function(){return this.__tM;
},changeActiveWindow:function(d,e){if(d){this.bringToFront(d);
d.setActive(true);
}
if(e){e.resetActive();
}},_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);
},syncWidget:function(){this.__tM.forceUnblockContent();
var f=this.__tM.getWindows();
...
}}this.__tM.setActiveWindow(k);
},bringToFront:function(n){var o=this.__tM.getWindows();
var p=qx.lang.Array.remove(o,n);

if(p){o.push(n);
this.updateStack();
}},sendToBack:function(q){var r=this.__tM.getWindows();
...
this.__tO=bf.left===undefined?0:bf.left;
this.__tN=bf.top===undefined?0:bf.top;
...
this.__tO=bg.left===undefined?0:bg.left;
this.__tN=bg.top===undefined?0:bg.top;
this.removeState(b);
this.hide();
this.fireEvent(m);
...
qx.ui.core.queue.Layout.add(this);
},_applyQuality:function(bf,bg){this.__Al(e,bf);
},_applyScale:function(bh,bi){this.__Al(a,bh);
},_applyWmode:function(bj,bk){this.__Al(O,bj);
},_applyPlay:function(bl,bm){this.__Al(s,bl);
},_applyLoop:function(bn,bo){this.__Al(J,bn);
},_applyMenu:function(bp,bq){this.__Al(L,bp);
},_applyAllowScriptAccess:function(br,bs){this.__Al(I,br);
},_applyLiveConnect:function(bt,bu){this.__Al(o,bt);
},__Al:function(bv,bw){this.getContentElement().setParam(bv,bw);
qx.ui.core.queue.Layout.add(this);
}}});
})();
...
this.__An={};
this.__Ao={};
},members:{__Am:null,__Ao:null,__An:null,__Ap:null,_createDomElement:function(){return qx.bom.Element.create(d);
},createFlash:function(){this.__Ap=qx.bom.Flash.create(this.getDomElement(),this.getAttributes(),this.__An,this.__Am);
},setSource:function(g){{};

if(this.__Ap){throw new Error("The source cannot be modified after initial creation");
}this.setAttribute(c,g);
},setId:function(h){{};

if(this.__Ap){throw new Error("The id cannot be modified after initial creation");
}this.setAttribute(b,h);
},getVariables:function(){return this.__An;
},setVariables:function(i){{};

if(this.__Ap){throw new Error("The variables cannot be modified after initial creation");
}this.__An=i;
},getAttributes:function(){return this.__Ao;
},setAttribute:function(j,k){{};

if(this.__Ap){throw new Error("The attributes cannot be modified after initial creation");
}
if(k===null||k===undefined){delete this.__Ao[j];
}else{this.__Ao[j]=k;
}},getParams:function(){return this.__Am;
},setParam:function(l,m){{};

if(this.__Ap){throw new Error("The params cannot be modified after initial creation");
}
if(m===null||m===undefined){delete this.__Am[l];
}else{this.__Am[l]=m;
}},getFlashElement:function(){return this.__Ap;
}},destruct:function(){if(this.__Ap){qx.bom.Flash.destroy(this.__Ap);
}this.__Am=this.__An=this.__Ao=null;
}});
})();
...
qx.Class.define(A,{statics:{_flashObjects:{},create:function(C,D,E,F,G){if(!G){G=window;
}{};

if(!D.width){D.width=h;
}
if(!D.height){D.height=h;
}F=F?qx.lang.Object.clone(F):{};

if(!F[f]){F[f]=D.movie;
}D[u]=D.movie;
delete D.movie;
if(E){for(var name in E){if(typeof F.flashvars!=l){F.flashvars+=r+name+e+E[name];
}else{F.flashvars=name+e+E[name];
}}}var H=this.__wu(C,D,F,G);
this._flashObjects[D.id]=H;
return H;
},destroy:qx.core.Variant.select(k,{"mshtml":function(I,J){I=this.__wp(I);

if(I.readyState==4){this.__wq(I);
}else{if(!J){J=window;
}qx.bom.Event.addNativeListener(J,y,function(){qx.bom.Flash.__wq(I);
});
}},"default":function(K,L){K=this.__wp(K);
K.parentNode.removeChild(K);
delete this._flashObjects[K.id];
}}),__wp:function(M){if(!M){throw new Error("DOM element is null or undefined!");
}
if(M.tagName.toLowerCase()!==j){M=M.firstChild;
}
if(!M||M.tagName.toLowerCase()!==j){throw new Error("DOM element has or is not a flash object!");
}return M;
},__wq:qx.core.Variant.select(k,{"mshtml":qx.event.GlobalError.observeMethod(function(N){for(var i in N){if(typeof N[i]==d){N[i]=null;
}}N.parentNode.removeChild(N);
delete this._flashObjects[N.id];
}),"default":null}),__wr:qx.event.GlobalError.observeMethod(function(){for(var O in qx.bom.Flash._flashObjects){qx.bom.Flash.destroy(qx.bom.Flash._flashObjects[O]);
}window.__ws=function(){};
window.__wt=function(){};
window.detachEvent(w,qx.bom.Flash.__wr);
}),__wu:qx.core.Variant.select(k,{"mshtml":function(P,Q,R,S){R.movie=Q.data;
delete Q.data;
delete Q.classid;
var T=t;

for(name in R){T+=z+name+m+R[name]+B;
}if(Q.id){P.innerHTML=p+Q.id+n+T+g;
delete Q.id;
}else{P.innerHTML=b+T+g;
}for(var name in Q){P.firstChild.setAttribute(name,Q[name]);
}return P.firstChild;
},"default":function(U,V,W,X){delete V.classid;
delete W.movie;
var ba=qx.bom.Element.create(j,V,X);
ba.setAttribute(x,c);
var Y;

for(var name in W){Y=qx.bom.Element.create(a,{},X);
Y.setAttribute(v,name);
Y.setAttribute(q,W[name]);
ba.appendChild(Y);
}U.appendChild(ba);
return ba;
}})},defer:function(bb){if(qx.core.Variant.isSet(k,s)){qx.bom.Event.addNativeListener(window,o,bb.__wr);
}}});
})();
